...
function checkMembershipStatus() {
    ...
    fetch('/membership/check-status', {
        ...
        body: JSON.stringify({ student_id: memberIdInput })
    })
    ...
    .then(data => {
        if (data.status === 'Not Found') {
            ...
            renewButton.style.display = data.status === "Expired" ? "inline-block" : "none";
        }
    })
    .catch(error => {
        console.error('Error:', error);
        resultDiv.textContent = "Error checking membership status: " + error.message;
        resultDiv.style.color = "red";  // Highlight error in red
    });
}
